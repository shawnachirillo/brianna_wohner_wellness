...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Invalid coaching content." },
        { status: 400 }
      );
    }
    ...
    if (!fileResponse.ok) {
      ...
      return NextResponse.json(
        { error: "Could not load the current coaching file." },
        { status: 500 }
      );
    }
    ...
    const saveResponse = await fetch(
      `https://api.github.com/repos/${OWNER}/${REPO}/contents/${FILE_PATH}`,
      {
        ...
        body: JSON.stringify({
          message: "Update coaching content",
    ...
    if (!saveResponse.ok) {
      ...
      return NextResponse.json(
        { error: "GitHub rejected the coaching update." },
        { status: 500 }
      );
    }
    ...
    console.error("Coaching publish error:", error);

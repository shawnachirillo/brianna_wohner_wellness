...
async function getCurrentFile(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Could not read home.json from GitHub (${response.status}): ${detail}`
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          error:
            "GitHub could not save the home page.",
        },
        {

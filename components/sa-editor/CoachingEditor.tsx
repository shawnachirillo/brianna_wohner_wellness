...
type Section = "Hero" | "Included" | "Real Fix" | "CTA";
type PublishStatus = "idle" | "publishing" | "success" | "error";

export default function CoachingEditor({ initialContent }: Props) {
  const [content, setContent] = useState<CoachingContent>(initialContent);
  ...
  function updateIncluded(
    field: keyof CoachingContent["included"],
    value: string | string[]
  ) {
    setContent((current) => ({
      ...current,
  ...
  function updateRealFix(
    field: keyof CoachingContent["realFix"],
    value: string | string[]
  ) {
    setContent((current) => ({
      ...current,
  ...
  function addHighlight() {
    updateHero("highlights", [...content.hero.highlights, ""]);
  }

  function removeHighlight(index: number) {
    updateHero(
      "highlights",
      content.hero.highlights.filter((_, itemIndex) => itemIndex !== index)
    );
  }

  function updateIncludedItem(index: number, value: string) {
    const items = [...content.included.items];
    items[index] = value;
    updateIncluded("items", items);
  }

  function addIncludedItem() {
    updateIncluded("items", [...content.included.items, ""]);
  }

  function removeIncludedItem(index: number) {
    updateIncluded(
      "items",
      content.included.items.filter((_, itemIndex) => itemIndex !== index)
    );
  }

  function updateRealFixParagraph(index: number, value: string) {
    const paragraphs = [...content.realFix.paragraphs];
    paragraphs[index] = value;
    updateRealFix("paragraphs", paragraphs);
  }

  function addRealFixParagraph() {
    updateRealFix("paragraphs", [...content.realFix.paragraphs, ""]);
  }

  function removeRealFixParagraph(index: number) {
    updateRealFix(
      "paragraphs",
  ...
  async function saveAndPublish() {
    try {
      ...
      setPublishMessage("Publishing changes...");

      const response = await fetch("/api/sa-editor/coaching", {
        method: "PUT",
        headers: {
  ...
  function HeroFields() {
    return (
      <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block">
            ...
            <input
              ...
              onChange={(e) =>
                updateHero("buttonText", e.target.value)
              }
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(e) =>
                updateHero("buttonLink", e.target.value)
              }
            />
          </label>
        </div>
        ...
        <label className="block">
          <span className={labelClass}>YouTube Embed URL</span>
          <input
            className={inputClass}
            value={content.hero.videoUrl}
            onChange={(e) =>
              updateHero("videoUrl", e.target.value)
            }
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(e) =>
              updateHero("videoTitle", e.target.value)
            }
          />
        </label>
      </div>
    ...
    return (
      <div className="space-y-5">
        <label className="block">
          ...
          <input
            className={inputClass}
            value={content.included.eyebrow}
            onChange={(e) =>
              updateIncluded("eyebrow", e.target.value)
            }
          />
        </label>
        ...
        <label className="block">
          ...
          <textarea
            rows={3}
            className={textareaClass}
            value={content.included.heading}
            onChange={(e) =>
              updateIncluded("heading", e.target.value)
            }
          />
        </label>
            ...
            {content.included.items.map((item, index) => (
              <div key={index} className="flex gap-2">
                <textarea
                  ...
                  value={item}
                  onChange={(e) =>
                    updateIncludedItem(index, e.target.value)
                  }
                />
    ...
    return (
      <div className="space-y-5">
        <label className="block">
          ...
          <input
            className={inputClass}
            value={content.realFix.eyebrow}
            onChange={(e) =>
              updateRealFix("eyebrow", e.target.value)
            }
          />
        </label>
        ...
        <label className="block">
          ...
          <textarea
            rows={3}
            className={textareaClass}
            value={content.realFix.heading}
            onChange={(e) =>
              updateRealFix("heading", e.target.value)
            }
          />
        </label>
        ...
        <div>
          ...
          <div className="space-y-3">
            {content.realFix.paragraphs.map((paragraph, index) => (
              <div key={index} className="flex gap-2">
                <textarea
                  rows={4}
                  className={textareaClass}
                  value={paragraph}
                  onChange={(e) =>
                    updateRealFixParagraph(index, e.target.value)
                  }
                />

                <button
                  type="button"
                  onClick={() => removeRealFixParagraph(index)}
                  className="self-start rounded-xl border border-red-200 px-4 py-3 text-sm text-red-600"
                >
                  Remove
                </button>
      ...
      <div className="space-y-5">
        <label className="block">
          ...
          <input
            ...
            onChange={(e) =>
              updateCTA("eyebrow", e.target.value)
            }
          />
        </label>
        ...
        <label className="block">
          ...
          <textarea
            ...
            onChange={(e) =>
              updateCTA("heading", e.target.value)
            }
          />
        </label>
  ...
  return (
    <div>
      <div className="mb-6">
        <p className="text-sm text-black/40">Coaching</p>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {(["Hero", "Included", "Real Fix", "CTA"] as Section[]).map(
          (section) => (
            <button
              key={section}
              type="button"
            ...
            >
              {section === "Included" ? "What's Included" : section}
            </button>
          )
        )}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
          {activeSection === "Hero" && HeroFields()}
          {activeSection === "Included" && IncludedFields()}
          {activeSection === "Real Fix" && RealFixFields()}
          {activeSection === "CTA" && CTAFields()}
        ...
        <div className="rounded-3xl border border-black/10 bg-brand-soft p-6 shadow-sm">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-black/40">
              Preview of the selected Coaching section.
            </p>
          </div>
